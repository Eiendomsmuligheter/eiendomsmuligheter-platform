...
set "totalFiles=0"
for /f "tokens=*" %%F in ('dir /s /b /a-d "%TEMP_DIR%"') do set /a totalFiles+=1
...
:: Kopier alle filer og mapper med deres struktur (unntatt .git mappen)
for /R "%TEMP_DIR%" %%F in (*) do (
    set "relativePath=%%~pF"
    set "relativePath=!relativePath:%TEMP_DIR%=!"
    
    :: Sjekk om dette er en .git-mappe
